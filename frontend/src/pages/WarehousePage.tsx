...
import type { BoxView, CellView, ShelfView } from '../api/types';
import { Button, Card, ConditionDot, EmptyState } from '../components/ui';

export default function WarehousePage() {
  const { depotId = '' } = useParams();
  const qc = useQueryClient();
  const [openBox, setOpenBox] = useState<BoxView | null>(null);
  ...
  return (
    <div className="space-y-6">
      ...
      {data?.shelves.map((shelf) => (
        <Shelf key={shelf.id} shelf={shelf} onOpenBox={setOpenBox} />
      ))}
        ...
        <section>
          ...
          <div className="flex flex-wrap gap-3">
            {data.freestandingBoxes.map((box) => (
              <BoxTile key={box.id} box={box} onClick={() => setOpenBox(box)} />
            ))}
          </div>
        </section>
      )}
          ...
          <Card className="divide-y divide-moos-50 p-2">
            {data.unassignedItems.map((it) => (
              <div key={it.id} className="flex items-center gap-2 px-2 py-1.5 text-sm">
                ...
                <span className="text-moos-400">×{it.quantity}</span>
              </div>
            ))}
          </Card>
        </section>
      ...
      {openBox && (
        <BoxDrawer depotId={depotId} box={openBox} onClose={() => setOpenBox(null)} />
      )}
    </div>
  );
}

function Shelf({ shelf, onOpenBox }: { shelf: ShelfView; onOpenBox: (b: BoxView) => void }) {
        ...
        {Array.from({ length: shelf.gridRows }).flatMap((_, r) =>
          Array.from({ length: shelf.gridCols }).map((__, c) => {
            ...
            return (
              <div
                key={`${r}-${c}`}
                className="min-h-[64px] rounded-lg border-2 border-zelt-300/70 bg-white/60 p-1"
              >
                {cell?.box && <BoxTile box={cell.box} onClick={() => onOpenBox(cell.box!)} compact />}
...
function BoxDrawer({
  ...
  const { data } = useQuery({
    queryKey: ['box-contents', depotId, box.id],
    queryFn: () => api.boxContents(depotId, box.id),
  });
  return (
    <div className="no-print fixed inset-0 z-20 flex items-end bg-black/30 sm:items-center sm:justify-center" onClick={onClose}>
      <div
        className="w-full rounded-t-2xl bg-white p-5 sm:max-w-md sm:rounded-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="mb-3 flex items-center justify-between">
          <h3 className="font-bold text-moos-800">📦 {box.label}</h3>
          <button onClick={onClose} className="text-moos-400" aria-label="Schließen">
            ✕
          </button>
        </div>
        {data && data.items.length === 0 && <p className="text-sm text-moos-400">Kiste ist leer.</p>}
        <ul className="max-h-72 space-y-1 overflow-auto">
          {data?.items.map((it) => (
            <li key={it.id} className="flex items-center gap-2 rounded-lg bg-moos-50 px-3 py-2 text-sm">
              <ConditionDot flag={it.conditionFlag} />
              <span className="flex-1">{it.name}</span>
              <span className="text-moos-400">×{it.quantity}</span>
            </li>
        ...
        <Link to={`/lager/${depotId}/druck/kiste/${box.id}`} target="_blank" className="mt-4 block">
